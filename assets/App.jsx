...
function ModalConfirmarPedido({ carrito, total, onConfirmar, onClose }) {
  ...
  const confirmar = async () => {
    ...
    setEnviando(true);
    try {
      // ── Calcular número de pedido del día (01–99) ──
      const hoy = new Date().toISOString().split("T")[0]; // "2026-04-22"
      const q = query(collection(db, "pedidos"), where("fechaPedido", "==", hoy));
      const snap = await getDocs(q);
      const numCorto = String(snap.size + 1).padStart(2, "0"); // "01", "07", "23"...
      ...
      const ref = await addDoc(collection(db, "pedidos"), {
        ...
        estado: "recibido",
        fechaPedido: hoy,        // 👈 fecha para agrupar por día
        numeroPedido: numCorto,  // 👈 número corto solo visible para el admin
        creadoEn: serverTimestamp(),
      });
 
      setCodigoPedido(ref.id);
      setNumeroPedido(numCorto);
      onConfirmar(ref.id);
    } catch (e) {
...
export default function App() {
  ...
  ¡Agrega unas rellenitas!<br /><br />

  <span style={{ fontSize: "20px", fontWeight: "bold" }}>
   ⚠️ Recuerda: si ya pediste, ve a Rastrear mi pedido y pon tu nombre.⚠️
  </span>
</p>
                </div>
              ) : carrito.map(item => (
